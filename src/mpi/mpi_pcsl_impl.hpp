 ...
 */
#ifndef MPI_PCSL_IMPL_HPP
#define MPI_PCSL_IMPL_HPP

#include "mpi_pcsl.hpp"
#include <mpi.h>
#include "../libsvm.hpp"
...
inline MPIProxCSL::MPIProxCSL(const std::string& trainBaseFilename,
...
inline void MPIProxCSL::TrainNaive()
{
...
inline void MPIProxCSL::Update(int max_iter, double alpha, bool adaptive)
{
  ...
  if (worldRank == 0)
  {
    ...
    std::cout << " - prox CSL solve time: " << CSLTime << "s.\n";
  }
}

inline double MPIProxCSL::TrainAccuracy()
{
...
inline double MPIProxCSL::TestAccuracy()
{
  // Compute accuracy on local system.
...
inline size_t MPIProxCSL::CountCorrect(const arma::sp_mat& data,
...
inline void MPIProxCSL::DistributeModel()
{
  // Send model from main worker to all other workers.
