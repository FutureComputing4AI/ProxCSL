                         ...
                         const arma::rowvec& labels)
{
  ...
  const double liblinearTime = c.toc();
  std::cout << "  LIBLINEAR conversion time: " << liblinearTime << "s.\n";
  ...
  const double liblinearResponseTime = c.toc();
  std::cout << "  LIBLINEAR response conversion time: " << liblinearResponseTime << "s.\n";
  ...
  const double dataCopyTime = c.toc();
  std::cout << "  Data internal copy time: " << dataCopyTime << "s.\n";
...
void GlobalStep<MatType>::Retrain()
{
  ...
  const double owaRoundOneTime = c.toc();
  ...
  #pragma omp critical
  {
    ...
    std::cout << " - Round one time: " << owaRoundOneTime << "s.\n";
    std::cout << " - Second round sample time: " << sampleTime << "s.\n";
...
void GlobalStep<MatType>::CSLUpdate(double lambda_t, double alpha)
{
  ...
  const double globalGradTime = c.toc();
  ...
  std::cout << "nonzeros " << modelNonzeros << std::endl;
  #pragma omp critical
  {
    std::cout << "CSL update timing for lambda " << lambda << ":\n";
    std::cout << " - Global gradient compute time: " << globalGradTime << "s.\n";
    std::cout << " - CSL update time: " << CSLTime << "s.\n";
  }
}
...
void GlobalStep<MatType>::DaneUpdate(double alpha)
{
  ...
  const double globalUpdateTime = c.toc();
  ...
  #pragma omp critical
  {
    std::cout << "DANE update timing for lambda " << lambda << ":\n";
    std::cout << " - Global gradient compute time: " << globalGradTime << "s.\n";
    std::cout << " - DANE global update time: " << globalUpdateTime << "s.\n";
  }
}

template<typename MatType>
void GlobalStep<MatType>::ProxCSLUpdate(double lambda_t, double alpha, bool adaptive)
{
  if (this->liblinearFeatures == nullptr)
  {
    throw std::invalid_argument("GlobalStep::ProxUpdate(): you must first call Train()!");
  }
  ...
  liblinear::solve_csl_cd(
    ...
    alpha,
    adaptive);
  const double solveCSLTime = c.toc();

  modelNonzeros = arma::accu(model != 0);

  #pragma omp critical
  {
    std::cout << "Global update timing for lambda " << lambda << ":\n";
    std::cout << " - Global gradient compute time: " << globalGradTime << "s.\n";
    std::cout << " - proximal update time: " << solveCSLTime << "s.\n";
  }
}

template<typename MatType>
void GlobalStep<MatType>::ProxQuadUpdate(double lambda_t, double alpha, bool adaptive)
{
  if (this->liblinearFeatures == nullptr)
  {
    throw std::invalid_argument("GlobalStep::ProxQuadUpdate(): you must first call Train()!");
  }
  ...
  liblinear::solve_csl_cd(
    ...
    alpha,
    adaptive);
  const double proxStepTime = c.toc();
  ...
  #pragma omp critical
  {
    std::cout << "Global update timing for lambda " << lambda << ":\n";
    std::cout << " - Global gradient compute time: " << globalGradTime << "s.\n";
    std::cout << " - proximal update time: " << proxStepTime << "s.\n";
  }
}
