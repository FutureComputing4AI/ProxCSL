...
{
  std::cout << "    MatToTransposedFeatureNodes, X size: " << X.n_rows << " x " << X.n_cols << "\n";
  arma::wall_clock c;
  c.tic();
  feature_node*** results = new feature_node**[partitions];
  const size_t batchSize = (X.n_cols + partitions - 1) / partitions;
  const double allocTime = c.toc();
  std::cout << "    Allocation time: " << allocTime << "s.\n";
  c.tic();
  MatType Xt = X.t();
  const double transTime = c.toc();
  std::cout << "    Transpose time: " << transTime << "s.\n";
  for (size_t p = 0; p < partitions; ++p)
  {
    ...
    const double partTime = c.toc();
    std::cout << "    Partition " << p << " to feature node time: " << partTime << "s.\n";
...
SubsampleMatToTransposedFeatureNodes(const MatType& X,
                                     ...
                                     const double ratio)
{
  std::cout << "    SubsampleMatToTransposedFeatureNodes, X size: " << X.n_rows << " x " << X.n_cols << "\n";
  arma::wall_clock c;
  c.tic();
  feature_node*** results = new feature_node**[partitions];
  const size_t batchSize = (X.n_cols + partitions - 1) / partitions;
                                     ...
                                     const size_t partitions)
{
  ...
  for (size_t p = 0; p < partitions; ++p)
  {
    ...
    const double partTime = c.toc();
    std::cout << "    Partition " << p << " to feature node time: " << partTime
        << "s.\n";
