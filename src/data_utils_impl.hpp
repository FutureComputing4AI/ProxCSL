                           ...
                           arma::rowvec& testLabels)
{
  ...
  const double orderTime = c.toc();
  std::cout << "Computing ordering for points took " << orderTime << "s."
      << std::endl;
  ...
  testLabels = labels.cols(order.subvec(lastTrainIndex,
      order.n_elem - 1));
  const double labelTime = c.toc();
  std::cout << "Splitting training and test labels took " << labelTime
      << "s." << std::endl;
  ...
  const double dataTime = c.toc();
  std::cout << "Splitting training and test data took " << dataTime << "s."
      << std::endl;

  const double splitTime = overallC.toc();
  std::cout << "Splitting dataset into training and test sets took "
      << splitTime << "s." << std::endl;
}
